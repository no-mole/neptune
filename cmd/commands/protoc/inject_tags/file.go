...
var (
	rComment = regexp.MustCompile(`^//.*?@(?i:cTags?|inject_tags?):\s*(.*)$`)
...
func ParseAndWrite(inputPath string, xxxSkip []string, verbose bool) error {
	_verbose = verbose
	areas, err := Parse(inputPath, xxxSkip)
	if err != nil {
		return err
	}
	err = writeFile(inputPath, areas)
	return err
}

func Parse(inputPath string, xxxSkip []string) (areas []textArea, err error) {
